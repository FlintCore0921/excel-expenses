package org.flintcore.excel_expenses.excels_handler.managers.routers.main;

import org.flintcore.excel_expenses.excels_handler.managers.exceptions.ErrorConsumerHandler;
import org.flintcore.excel_expenses.excels_handler.managers.routers.IRouter;
import org.flintcore.excel_expenses.excels_handler.managers.routers.RouteManager;
...
public class MainViewRouter implements IRouter<EMainRoute> {
    ...
    public void navigateBack(Consumer<Node> paneConsumer, Function<Node, Transition> transition) {
        this.routerManager.canNavigateBack();
        this.navigateTo(this.routerManager.currentRoute(), paneConsumer, transition);
    }

    @Override
    public void navigateBack() {
        if (!this.routerManager.canNavigateBack()) {
            return;
        }

        this.routerManager.navigateTo(this.routerManager.previousRoute());
    }
}
